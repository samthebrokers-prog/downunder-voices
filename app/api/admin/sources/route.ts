...
export async function POST(request: Request) {
  ...
  try {
    ...
    const defaultCategory = cleanText(
    ...
    if (
      !name ||
      !validHttpUrl(feedUrl) ||
      !categories.some(
        (category) =>
          category.slug === defaultCategory,
      )
    ) {
      return NextResponse.json(
        {
          error:
            'Please enter a name, valid feed URL and category.',
        },
        { status: 400 },
      )
    }
    ...
    console.error('Add RSS source failed:', error)
