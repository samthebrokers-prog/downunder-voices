...
function attribute(block: string, tagName: string, attributeName: string) {
  ...
  const match = block.match(
    new RegExp(`<${escaped}\\b[^>]*\\b${attributeName}=["']([^"']+)["'][^>]*>`, 'i'),
  )
...
function validImageUrl(value?: string) {
  ...
  if (/\.(mp3|mp4|m4a|wav|ogg|pdf)(\?|$)/i.test(cleaned)) return undefined
  return cleaned
}
...
function imageFromBlock(block: string): string | undefined {
  ...
  const candidates = [
    ...
    rawDescription.match(/<img[^>]+(?:src|data-src|data-original)=["']([^"']+)["']/i)?.[1],
    block.match(/<img[^>]+(?:src|data-src|data-original)=["']([^"']+)["']/i)?.[1],
  ]
...
export async function fetchFeed(feedUrl: string): Promise<FeedItem[]> {
  const response = await fetch(feedUrl, {
    headers: { 'User-Agent': 'DownunderVoicesBot/1.0 (+https://downundervoices.com)' },
  ...
  return blocks
    .map((block) => {
      ...
      const rawDescription = tag(block, ['content:encoded', 'description', 'summary', 'content'])
      const description = stripHtml(rawDescription).slice(0, 1200)
      const rawDate = stripHtml(tag(block, ['pubDate', 'published', 'updated', 'dc:date']))
      ...
      return {
        ...
        link,
        description,
        publishedAt: Number.isNaN(parsedDate.valueOf()) ? new Date().toISOString() : parsedDate.toISOString(),
...
export function classifyCategory(title: string, description: string, fallback: CategorySlug) {
  const haystack = `${title} ${description}`.toLowerCase()
  const rules: Array<[CategorySlug, RegExp]> = [
    ['sports', /sport|rugby|cricket|football|soccer|netball|league|afl|olympic/],
    ['business', /business|economy|company|jobs|employment|bank|interest rate|inflation|trade/],
    ['politics', /government|parliament|minister|election|policy|council|politic/],
    ['community', /community|volunteer|charity|school|family|health|housing|local/],
    ['australia', /australia|australian|sydney|melbourne|perth|brisbane|canberra/],
    ['nz-pacific', /new zealand|aotearoa|auckland|wellington|pacific|fiji|tonga|samoa/],
  ]
  return rules.find(([, rule]) => rule.test(haystack))?.[0] ?? fallback
}
